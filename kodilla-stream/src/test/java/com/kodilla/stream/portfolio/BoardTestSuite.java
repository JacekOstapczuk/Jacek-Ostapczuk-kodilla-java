...
class BoardTestSuite {
    ...
    @Test
    void testAddTaskListAverageWorkingOnTaskInTwoLines(){
        //Given
        Board project = prepareTestData();


        //When
        List<TaskList> averageProgressDays = new ArrayList<>();
        averageProgressDays.add(new TaskList("In progress"));

       int sumOfDays = project.getTaskLists().stream()
                .filter(averageProgressDays::contains)
                .flatMap(apd -> apd.getTasks().stream())
                .map((Task::getCreated) )
                .map( d-> d.compareTo( LocalDate.now())  )
                .reduce(0, (sum, current) -> sum = sum +current);


        int sumOfTasks = project.getTaskLists().stream()
                .filter(averageProgressDays::contains)
                .flatMap(apd -> apd.getTasks().stream())
                .map(n->1 )
                .reduce(0, (sum, current) -> sum = sum +current);
            double averange =  sumOfDays /sumOfTasks;


        //Then

        assertEquals(-10,  averange);

    }
/*

    @Test
    void testAddTaskListAverageWorkingOnTaskInOneLines(){
        //Given
        Board project = prepareTestData();


        //When
        List<TaskList> averageProgressDaysII = new ArrayList<>();
        averageProgressDaysII.add(new TaskList("In progress"));

        List average =  project.getTaskLists().stream()
                .filter(averageProgressDaysII::contains)
                .flatMap(apd -> apd.getTasks().stream())
                .map((Task::getCreated) )
                .map( d-> d.compareTo( LocalDate.now())  )
